...
switch ($op){
    ...
    $query = "SELECT MIN(id_nilaisupplier) AS id_nilaisupplier, 
                     id_supplier, 
                     supplier.namaSupplier AS namaSupplier, 
                     jenis_barang.id_jenisbarang AS id_jenisbarang, 
                     jenis_barang.namaBarang AS namaBarang 
              FROM nilai_supplier 
              INNER JOIN supplier USING(id_supplier) 
              INNER JOIN jenis_barang USING (id_jenisbarang) 
              $where 
              GROUP BY id_supplier, jenis_barang.id_jenisbarang 
              ORDER BY id_jenisbarang, id_supplier ASC";
            ...
            <tr id='data'>
                ...
                <td>
                    <div class='norebuttom'>
                        ...
                        <a class=\"btn btn-yellow\" data-a=\"$data[namaBarang] - $data[namaSupplier]\" id='hapus' href='./proses/proseshapus.php/?op=nilai&id=$data[id_supplier]'><i class='fa fa-trash-alt'></i></a>
                    </div>
                </td>
            </tr>";
            $no++;
